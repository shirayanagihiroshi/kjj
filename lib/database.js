...
bulkWrite = async function (colName, queryObj, outputFieldObj, callback) {
  const database = client.db(dbName);
  const collection = database.collection(colName);

  const result = await collection.bulkWrite(queryObj, outputFieldObj);
  callback(result);
}
